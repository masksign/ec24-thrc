...
//  === From XKCP (eXtended Keccak Code Package) https://github.com/XKCP/XKCP
...
#define LOAD4_64(a, b, c, d)                                       \
    _mm256_set_epi64x((uint64_t)(a), (uint64_t)(b), (uint64_t)(c), \
                      (uint64_t)(d))
#define ROL64in256(d, a, o) \
    d = _mm256_or_si256(_mm256_slli_epi64(a, o), _mm256_srli_epi64(a, 64 - (o)))
#define ROL64in256_8(d, a) d = _mm256_shuffle_epi8(a, CONST256(rho8))
#define ROL64in256_56(d, a) d = _mm256_shuffle_epi8(a, CONST256(rho56))
static XALIGN(KeccakP1600times4_statesAlignment) const uint64_t rho8[4] = {
    0x0605040302010007, 0x0E0D0C0B0A09080F, 0x1615141312111017,
    0x1E1D1C1B1A19181F};
static XALIGN(KeccakP1600times4_statesAlignment) const uint64_t rho56[4] = {
    0x0007060504030201, 0x080F0E0D0C0B0A09, 0x1017161514131211,
    0x181F1E1D1C1B1A19};
                                ...
                                unsigned int length)
{
    ...
    if ((sizeLeft > 0) && (offsetInLane != 0)) {
        ...
        memcpy((uint8_t *)&lane + offsetInLane, curData, bytesInLane);
...
void KeccakP1600times4_AddLanesAll(void *states, const uint8_t *data,
...
void KeccakP1600times4_OverwriteLanesAll(void *states, const uint8_t *data,
                                         unsigned int laneCount,
                                         unsigned int laneOffset)
{
    V256 *stateAsLanes = (V256 *)states;
...
void KeccakP1600times4_ExtractBytes(const void *states,
                                    unsigned int instanceIndex, uint8_t *data,
...
void KeccakP1600times4_ExtractLanesAll(const void *states, uint8_t *data,
                                       unsigned int laneCount,
                                       unsigned int laneOffset)
{
                                          ...
                                          unsigned int length)
{
    ...
    unsigned int lanePosition = offset / SnP_laneLengthInBytes;
    unsigned int offsetInLane = offset % SnP_laneLengthInBytes;
    const uint8_t *curInput = input;
    uint8_t *curOutput = output;
    const uint64_t *statesAsLanes = (const uint64_t *)states;
    ...
    if ((sizeLeft > 0) && (offsetInLane != 0)) {
        ...
        sizeLeft -= bytesInLane;
        do {
            *(curOutput++) = *(curInput++) ^ (uint8_t)lane;
            lane >>= 8;
        } while (--bytesInLane != 0);
        lanePosition++;
    }
...
static XALIGN(KeccakP1600times4_statesAlignment) const uint64_t
...
#undef Xor_In4
        ...
        return (const uint8_t *)curData0 - dataStart;
#else
        // unsigned int i;
        const uint8_t *dataStart = data;
        const uint64_t *curData0 = (const uint64_t *)data;
...
#undef XOR_In
        ...
        copyToState(statesAsLanes, A) return (const uint8_t *)curData0 -
            dataStart;
#endif
    } else {
        // unsigned int i;
...
size_t KeccakP1600times4_12rounds_FastLoop_Absorb(
    ...
    unsigned int laneOffsetSerial, const uint8_t *data, size_t dataByteLen)
{
    if (laneCount == 21) {
#if 0
        const uint8_t *dataStart = data;
...
#define ExtrAccu4(lanes0, lanes1, lanes2, lanes3, p, argIndex)             \
    UNINTLEAVEa(lanes0, lanes1, lanes2, lanes3), XOReq256(lanes0, lanes1), \
        ...
        lanes1 = LOAD256u(p[argIndex]), XOReq256(lanes0, lanes2),          \
        XOReq256(lanes0, lanes1), STORE256u(p[argIndex], lanes0)
...
#if defined(UseGatherScatter)
    V128 gather =
        _mm_setr_epi32(0 * 25 * 8, 1 * 25 * 8, 2 * 25 * 8, 3 * 25 * 8);
#endif
...
#define ExtrAddKey(lanes, p, argIndex)                         \
    XOReq256(lanes, CONST256_64(kRoll[argIndex])),             \
        p[argIndex + 0 * 25] = _mm256_extract_epi64(lanes, 0), \
                         p[argIndex + 1 * 25] =                \
                             _mm256_extract_epi64(lanes, 1),   \
                         p[argIndex + 2 * 25] =                \
                             _mm256_extract_epi64(lanes, 2),   \
